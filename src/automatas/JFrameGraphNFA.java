 ...
 * @author Allan Mejia
 */
public class JFrameGraphNFA extends JFrame {
    ...
    private javax.swing.JMenuItem jMenuItemConvertiraDFA;
    ...
    protected static NFA nfa;
    
    public JFrameGraphNFA() {
        initComponents();
        nfa=new NFA();
    }
    ...
    JFrameGraphNFA(NFA nfaeConvertido) {
        initComponents();
        NewEstadoNFA crearEstados=new NewEstadoNFA(nfaeConvertido);
        NewTransitionNFA crearTrasiciones=new NewTransitionNFA(nfaeConvertido);
        graph.getModel().setStyle(nfaeConvertido.estadoInicial.vertex, "resizable=0;editable=0;shape=ellipse;whiteSpace=wrap;"
        ...
        for(Estado estado:nfaeConvertido.estadosFinales){
            if(estado.nombreEstado.equals(nfaeConvertido.estadoInicial.nombreEstado)){
    ...
    private void initComponents() {
        ...
        jMenuItemConvertiraDFA = new javax.swing.JMenuItem();
        ...
        graphComponent.getConnectionHandler().addListener(mxEvent.CONNECT, (Object sender, mxEventObject evt) -> {
            ...
            edge.setValue(name);
            JFrameGraphNFA.nfa.AddTransition(v1,v2,name,edge);
        });
        ...
        jMenuItemConvertiraDFA.setText("Convertir a DFA");
        jMenuItemConvertiraDFA.addActionListener((ActionEvent e) -> {
                convertiraDFA(e);
        });
        jMenuConversion.add(jMenuItemConvertiraDFA);
    ...
    private void agregarEstado(java.awt.event.ActionEvent evt) {                                                
       String name = String.valueOf(valor++);
       NewEstadoNFA add = new NewEstadoNFA(name);
    }
    ...
    private void agregarTrasicion(java.awt.event.ActionEvent evt) {                                                
        ...
        NewTransitionNFA add= new NewTransitionNFA(v1,v2,nombre);
    }
    ...
    private void eliminarEstado(Estado v1) {
        if(nfa.estadoInicial.nombreEstado.equals(v1.nombreEstado)){
            nfa.estadoInicial=new Estado();
        }
    ...
    private void eliminarSiEsEstadoFinal(Estado v){
        for(Estado estado:nfa.estadosFinales)
        {
            ...
            {
                nfa.estadosFinales.remove(estado);
                break;
            }
        }
    }
    private void eliminarTransiciones(Estado v){
        ...
        nfa.transiciones.stream().filter((transicion) -> (transicion.origen.nombreEstado.equals(v.nombreEstado) || 
                transicion.destino.nombreEstado.equals(v.nombreEstado))).forEach((transicion) -> {
            eliminarTransiciones.add(transicion);
        });
        nfa.transiciones.removeAll(eliminarTransiciones);
    }
    ...
    private boolean siEsEstadoInicial(Estado v1) {
        if (v1.nombreEstado.equals(nfa.estadoInicial.nombreEstado)) {
            graph.getModel().setStyle(v1.vertex, "resizable=0;editable=0;shape=doubleEllipse;whiteSpace=wrap;"
                    +"fillColor=lightgreen");
            nfa.estadosFinales.add(v1);
            return true;
    ...
    private Transicion obtenerTransicionAEliminar(Estado v1, Estado v2, char nombre) {
        for(Transicion transicion:nfa.transiciones){
            if(transicion.origen.nombreEstado.equals(v1.nombreEstado)&&
                ...
                nfa.transiciones.remove(transicion);
                return t1;
            }
        }
        return null;
    ...
    private void probarCadenaEnAutomata(ActionEvent e) {
        ...
        if(nfa.EvaluarCadena(jTextFieldCadena.getText())){
    ...
    private boolean prologoAntesDeProbar() {
        ...
        if (nfa.estadoInicial.nombreEstado.isEmpty()) {
            showMessage("Debe existir un estado inicial!");
            return true;
        }
        if(nfa.estadosFinales.isEmpty()){
    ...
    private boolean seCreaAlfabeto(char [] alfabeto){
        ...
        if(!nfa.alfabeto.isEmpty()){
            nfa.alfabeto=new ArrayList<>();
        }
        for(int i = 0;i<alfabeto.length;i++){
            nfa.alfabeto.add(alfabeto[i]);
        }
        return true;
    }
    ...
    private void convertiraDFA(ActionEvent e) {
        ...
        DFA NFAConvertido = nfa.ConversionADFA();
        JFrameGraph frame = new JFrameGraph(NFAConvertido);
    ...
    private boolean prologoAntesDeConvertir() {
        ...
        if (nfa.estadoInicial.nombreEstado.isEmpty() ||
                nfa.estados.isEmpty() || nfa.estadosFinales.isEmpty()
                || nfa.transiciones.isEmpty()) {
