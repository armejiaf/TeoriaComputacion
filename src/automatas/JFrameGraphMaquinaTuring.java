 ...
 * @author Allan Mejia
 */
public class JFrameGraphMaquinaTuring extends javax.swing.JFrame{
    ...
    protected static MaquinaTuring mdt;
    
    public JFrameGraphMaquinaTuring() {
        initComponents();
        mdt=new MaquinaTuring();
    }
    ...
    private void initComponents() {
        ...
        graphComponent.getConnectionHandler().addListener(mxEvent.CONNECT, (Object sender, mxEventObject evt) -> {
            ...
            JFrameGraphMaquinaTuring.mdt.AddTransition(v1,v2,nombre,edge);
        });
        ...
        jLabel5.setText("TIP: Para escoger la direccion en que se mueve la cinta utilize R para Derecha o L para Izquierda.");
    ...
    private void agregarEstado(java.awt.event.ActionEvent evt) {                                                
       String name = String.valueOf(valor++);
       NewEstadoMaquinaTuring add = new NewEstadoMaquinaTuring(name);
    }
    ...
    private void agregarTrasicion(java.awt.event.ActionEvent evt) {                                                
        ...
        NewTransitionMaquinaTuring add= new NewTransitionMaquinaTuring(v1,v2,nombre);
    }
    ...
    private void eliminarEstado(Estado v1) {
        if(mdt.estadoInicial.nombreEstado.equals(v1.nombreEstado)){
            mdt.estadoInicial=new Estado();
        }
        graph.getModel().remove(v1.vertex);
    }

    private Estado estadoAEliminar(String name) {
        for(Estado estado:mdt.estados)
        {
            if(estado.nombreEstado.equals(name))
            {
                Estado v1 = estado;
                mdt.estados.remove(estado);
                return v1;
            }
        }
        return null;
    ...
    private void eliminarSiEsEstadoFinal(Estado v){
        for(Estado estado:mdt.estadosFinales)
        {
            ...
            {
                mdt.estadosFinales.remove(estado);
                break;
            }
        }
    }
    private void eliminarTransiciones(Estado v){
        ...
        mdt.transiciones.stream().filter((transicion) -> (transicion.origen.nombreEstado.equals(v.nombreEstado) || 
                transicion.destino.nombreEstado.equals(v.nombreEstado))).forEach((transicion) -> {
            eliminarTransiciones.add(transicion);
        });
        mdt.transiciones.removeAll(eliminarTransiciones);
    }
    ...
    private void convertirAEstadoFinal(Estado v1) {
        graph.getModel().setStyle(v1.vertex, "resizable=0;editable=0;shape=doubleEllipse;whiteSpace=wrap;"
                +"fillColor=lightblue");
        mdt.estadosFinales.add(v1);
    }

    private boolean siEsEstadoInicial(Estado v1) {
        if (v1.nombreEstado.equals(mdt.estadoInicial.nombreEstado)) {
            graph.getModel().setStyle(v1.vertex, "resizable=0;editable=0;shape=doubleEllipse;whiteSpace=wrap;"
                    +"fillColor=lightgreen");
            mdt.estadosFinales.add(v1);
            return true;
    ...
    private Transicion obtenerTransicionAEliminar(Estado v1, Estado v2, String nombre) {
        for(Transicion transicion:mdt.transiciones){
            if(transicion.origen.nombreEstado.equals(v1.nombreEstado)&&
                ...
                mdt.transiciones.remove(transicion);
                return t1;
            }
        }
        return null;
    ...
    private String escogerTransicion(){
        ...
        while(true){
            ...
            if(na.length!=3){
                ...
                if(na[1].length()!=1){
                    showMessage("El valor a cambiar en la cinta debe ser de un caracter!");
                    continue;
                }
                if(na[2].equals("R") || na[2].equals("L")){
                     nombre=name;
                    break;
                }
                showMessage("Ingreso un caracter extraño:"+ na[2]+"\nLos valores para mover cinta son: R (Derecha) o  L (Izquierda)");
            }
        }
        return nombre;
    }
    ...
    private void probarCadenaEnAutomata(ActionEvent e) {
        ...
        if(!seCreaAlfabeto(jTextFieldAlfabeto.getText().toCharArray()))
            return;
        String cadenaAEvaluar = jTextFieldCadena.getText();
        llenarCinta(cadenaAEvaluar);
        
        if(mdt.EvaluarCadena(cadenaAEvaluar)){
    ...
    private void llenarCinta(String cadenaAEvaluar) {
        if(!mdt.cinta.isEmpty()){
            mdt.cinta=new ArrayList<>();
            mdt.posicionActualCinta=5;
        }
        for(int i = 0;i < 5;i++)
            mdt.cinta.add("B");
        for(int i=0;i < cadenaAEvaluar.length();i++)
            mdt.cinta.add(String.valueOf(cadenaAEvaluar.charAt(i)));
        for(int i = 0;i < 5;i++)
            mdt.cinta.add("B");
    }
    ...
    private boolean prologoAntesDeProbar() {
        ...
        if (mdt.estadoInicial.nombreEstado.isEmpty()) {
            showMessage("Debe existir un estado inicial!");
            return true;
        }
        if(mdt.estadosFinales.isEmpty()){
    ...
    private boolean seCreaAlfabeto(char [] alfabeto){
        ...
        if(!mdt.alfabeto.isEmpty()){
            mdt.alfabeto=new ArrayList<>();
        }
        for(int i = 0;i<alfabeto.length;i++){
            mdt.alfabeto.add(alfabeto[i]);
        }
        return true;
    }
